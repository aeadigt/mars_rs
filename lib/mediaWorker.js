...
function init() {
    ...
    session.timeOutIDgetStream = setTimeout(function () {
        if (!session.stream_on) {
            bus.emit('message', { category: 'call', sessionID: sessionID, type: 'error', msg: 'RTP process timeout timeOutIDgetStream' });
            session.close();
        }
    }, 180000); //3 min

    worker.on('message', function (d) {
        if ( !d || ( d && d.action === undefined) ) {
    ...
    session.close = function () {
        ...
        clearTimeout(session.timeOutID);
        clearTimeout(session.timeOutIDgetStream);
    };
