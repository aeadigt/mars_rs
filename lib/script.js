...
function onAnswered() {
    ...
    function ttsPlay(o, cb){
        bus.emit('tts', {sipAccountID: sipAccountID, sessionID: sessionID, rewrite: o.rewrite, type: o.type, voice: o.voice, key:o.key, text: o.text, cb: function (fileName) {
            ttsFile = fileName;
            if (!ttsFile)
            {
                if (cb)
            ...
            switch (k) {
                ...
                case 'wait':
                    f = function () {
                        ...
                        session.timerId = setTimeout(function () {
                            console.log('Сработал wait в сценарии', o.time * 1000);
                            if (cb) {
                                cb();
                            }
                        }, o.time * 1000);
                ...
                case 'tts':
                    f = function () {
                        ...
                        bus.emit('tts', {sipAccountID: sipAccountID, sessionID: sessionID, rewrite: o.rewrite, type: o.type, key: o.key, voice: o.voice, text: o.text, cb: function (fileName) {
                                ttsFile = fileName;
                                if (cb)
                                    cb();
                            }});
