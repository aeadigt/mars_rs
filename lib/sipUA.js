...
function init() {
    ...
    sip.sendRq = function (rq, cb, sessionID, sipAccountID) {
        ...
        sip.connections[sipAccountID].sUA.send(rq, function (rs) { 
            ...
            if (rs.status >= 300) {
                    ...
                    sip.connections[sipAccountID].sUA.send(rq, function (rs_) {
                        ...
                        if (rs_.status < 200) {
                            ...
                            if (rs_.status === 200 || rs_.status === 487) { //487 for CANCEL
                                bus.emit('message', { type: 'debug', category: logCategory, msg: 'SIP method "' + rq.method + '" successfully authorized 200 or 487 status' });
                                if (cb) {
                                    cb(rs_);
                                    cb = null;
                                }
                ...
                bus.emit('message', { type: 'debug', category: logCategory, msg: 'SIP method "' + rq.method + '" successfully authorized else branch' });
                if (cb) {
                    cb(rs);
                    cb = null;
                }
    ...
    function startSip(sipAccountID) {
        ...
        return sip.create({
            ...
            logger: {
                recv: function (m, i) { 
                    bus.emit('message',({ type: 'debug', category: 'sip', msg: 'RECV from ' + i.protocol + ' ' + i.address + ':' + i.port + '\n' + sip.stringify(m)})); 
                },
                send: function (m, i) {
                    bus.emit('message', { type: 'debug', category: 'sip', msg: 'SEND to ' + i.protocol + ' ' + i.address + ':' + i.port + '\n' + sip.stringify(m) });
                },
                error: function (e) {
                    bus.emit('message', { type: 'error', category: 'sip', msg: 'register: ' + err });
                }
            }
