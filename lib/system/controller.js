...
function init() {
    ...
    bus.on('message', function (data) {
        data['category'] = data.category || 'server';
        if (!logger) {
            logger = bus.getLogger(data['category']);
        }
    ...
    bus.on('setOnEvent', function (data) {
        /*
        if (bus.OnEvent[data.event] === undefined) {
            bus.emit('message', {type: 'error', msg: 'Event "' + data.event + '" not available'});
            return;
        }
        */

        for (var key in bus.OnEvent) {
            if (bus.OnEvent[key] == data.script) {
                delete bus.OnEvent[key];
            }
        }
        bus.OnEvent[data.event] = data.script;
        //var sipAccountID = data.event.replace(/\w+(\d+)$/, '$1');
        bus.emit('message', { type: 'debug', msg: 'setOnEvent "' + data.event + '" script "' + data.script + '"' });
    });
    ...
    function startScript(params) {
        ...
        var script_dir = config.get("scriptDir") || './scripts',
            ...
            if (params.rejectTime && new Date(params.rejectTime).getTime() < new Date().getTime()) {
        ...
        var isForcedStart = false;

        if (params && params.params && Array.isArray(params.params) ) {
            isForcedStart = params.params.some((item) => {
                return (item == 'forcedStart');
            });
        }
        ...
        if (sip.dialogs && sipAccountLines <= cur_acc_dialogs && !isForcedStart) {
        ...
        } else if (sip.dialogs && callLimit() && !isForcedStart) {
